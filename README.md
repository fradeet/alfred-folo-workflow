...
- Read-only Folo CLI responses (timeline, subscriptions, unread list) are cached as JSON
  files in Alfred's workflow cache (`folo-requests/`). The filename is a stable hash of the
  CLI arguments, and each Script Filter response reports it in the `frr_result_cache_key`
  workflow variable. Cached responses are served until Alfred's cache is cleared.
